...
import { checkIfTwoWordMatch } from "../src/wordleAlgorithm";
...
describe("checkIfTwoWordMatch", () => {
  ...
  it("This test see if the return value has 5 letter", () => {
    ...
    expect(resultArray.some((letter) => letter.hasOwnProperty("letter"))).toBe(
      true
    );
    expect(resultArray.some((letter) => letter.hasOwnProperty("result"))).toBe(
      true
    );
  });
